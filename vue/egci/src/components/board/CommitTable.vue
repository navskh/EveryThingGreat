<template>
  <div class="card">
    <div class="scroll">
      <h1 class="text-2xl font-bold my-2">Commit Log</h1>
      <div class="table-container">
        <div class="table-row">
          <div class="day" style="font-size: 8px"></div>
          <div class="day" style="font-size: 8px" v-for="day of days"> {{ day }}</div>
        </div>
        <div v-for="(weekArr, dlength) of sampleDate" class="table-row">
          <div class="month"> {{ weekArr.month ?? " " }} </div>
          <div v-for="(day, index) of weekArr.week"
            :class="'committed-' + (day.commit > 3 ? '3' : day.commit) + ' table-cell tooltip'"
            :style="dlength == sampleDate.length - 1 ? 'flex: 0' : ''">
...
const init = async () => {
  ...
  while (seq < 365) {
    ...
    for (let i = seq; i < seq + 7; i++) {
      ...
      const commit = crDateList.filter(crDate => crDate == dateString).length;
      const dateStringAndCommit = `${makeDayString(thisDate)} (${commit})`;
      weekArr.push({ date: dateStringAndCommit, commit: commit });
      seq++;
      if (dateToDay(thisDate) == 'Sun') break;
    }
    if (curMonth == prevMonth) dateArr.push({ month: null, week: weekArr.reverse() });
    else {
